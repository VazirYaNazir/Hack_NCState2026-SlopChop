...
_TWEET_CACHE_TTL_S = 20 * 60  # 20 minutes

# =========================================================
# Robust .env loading (works when run from src/ or via uvicorn)
# =========================================================
...
# =========================================================
# Import ai_engine.py from 1 level up (../ai_engine.py)
# backend/src/xapi.py -> backend/ai_engine.py
# =========================================================
...
# Bearer token (X v2 app-only read)
...
# cache for your expensive model calls
...
def _is_probably_english(s: str) -> bool:
    ...
    if not s:
        return True
    # CJK / Hangul / Hiragana / Katakana ranges
...
# =========================================================
# Google Trends -> topic queries (English)
# =========================================================
...
# =========================================================
# X v2 search: REAL tweets with REAL media URLs
# ========================================================
def _topic_query_variants(topic: str) -> List[str]:
    ...
    # Remove problematic characters for search (keep it simple)
    t_clean = re.sub(r"[^\w\s#@-]", " ", t).strip()
    t_clean = re.sub(r"\s+", " ", t_clean)

    # Prefer exact phrase when it’s multiword, else raw token
    ...
    return [
        ...
        # loosen to media (video/gif previews still give image URLs)
        f"{phrase} has:media lang:en -is:retweet",
        f"{t_clean} has:media lang:en -is:retweet",
        # last resort: plain query; we still require media_keys in code
...
def search_x_tweets_with_media(topic: str, per_topic: int = 2) -> List[Dict[str, Any]]:
    ...
    # Try a few variants; stop as soon as we have enough
    for q in _topic_query_variants(topic):
        ...
        for attempt in range(3):
            try:
                ...
                # If rate-limited, return cache if available; otherwise raise
                last_error = e
                if cached and cached[1]:
                    return cached[1][:per_topic]
                raise
            except (tw.errors.Unauthorized, tw.errors.Forbidden) as e:
                # Access/tier/auth issue: do not hide it behind empty results
                raise
        ...
        for twt in tweets:
            ...
            # Choose a direct media URL (photo preferred)
            image_url = None
        ...
        if out:
            # Cache last-good and return
            _TWEET_CACHE[topic] = (now, out)
            return out[:per_topic]

    # If we got here, nothing worked. Prefer cache if it exists even if stale.
    if cached and cached[1]:
        return cached[1][:per_topic]

    # Otherwise, surface the failure instead of silently returning empty
    if last_error:
        raise last_error
...
# =========================================================
# Main: trends -> tweets -> posts schema (with your functions)
# =========================================================
def get_posts_from_trends_as_real_tweets(
    ...
    for ev in trends_payload["trends"]:
        ...
        for h in hits:
            ...
            # optional fusion (keeps your keys the same)
            risk_score = max(risk_score, int(round(ai_prob * 100)))

            posts.append(
                {
                    "id": tid,                       # tweet id (real)
                    "username": h["username"],       # tweet author (real)
                    "image_url": image_url,          # direct media url (real)
                    "caption": caption,              # tweet text (real)
                    "likes": int(h["likes"]),
...
if __name__ == "__main__":
    ...
    # Each tweet URL can be built from your existing fields:
